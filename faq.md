...
PrivaPlane は、顧客が管理できる環境で利用することを前提にしています。典型的には、顧客側の認証入口の背後、社内ネットワーク、オンプレミス環境、またはプライベートエッジ環境で動かす形を想定します。

ただし、試験導入や特殊な構成で外部サービスを使う必要がある場合は、事前に範囲、送信対象、責任分担を明確にします。標準説明としては「顧客管理環境を前提に設計する」と理解してください。
...
- 最初に扱う資料セット
- 利用予定者または検証担当者
- 実行環境。例: MacBook、Mac Studio、GPU 搭載 Linux サーバー、社内ネットワーク上のオンプレミスサーバー、プライベートエッジ環境
- 認証入口やアクセス制御の考え方
- 受け入れ時に確認したい質問例
...
SKU A は、限定された資料セットで社内資料 Q&A を始めるための基本プランです。まず回答品質、根拠表示、利用フローを確認したい場合に向いています。

SKU B は、複数部署、複数ナレッジベース、品質確認、監査・受け入れ証跡、バックアップ・復旧確認、運用支援まで含めたい場合に向いています。
...
SKU B では、監査出力、受け入れ証跡、バックアップ・復旧確認、運用記録を含めて検討できます。ただし、具体的な出力項目や復旧手順は顧客環境に合わせて確認します。

### Q8. 顧客ローカル環境への交付や Mac / Linux サーバーでの実行はできますか？

現在の設計では、PrivaPlane は顧客側のローカル環境に交付し、その環境内で動かす形を重視しています。候補環境としては、MacBook または Mac Studio 上で 7B〜14B クラスのローカルモデルを動かす構成、または顧客が用意する GPU 搭載 Linux サーバー上で動かす構成を確認できます。
...
PrivaPlane is designed around customer-controlled environments. A typical deployment can run behind customer-controlled authentication ingress on a private network, on-premise server, or private edge node.

If a trial or special configuration requires any external service, the scope, transmitted data, and responsibility boundary should be confirmed before use. The default product framing is customer-controlled private deployment.
...
- Runtime environment, such as MacBook, Mac Studio, customer-provided GPU Linux server, on-premise server on a private network, or private edge node
...
SKU A is the basic package for starting private knowledge Q&A with a bounded document set. It is suitable when the first goal is to validate answer quality, source review, and usage flow.

SKU B is for teams that need multiple departments, multiple knowledge bases, quality review, audit or acceptance evidence, backup/restore checks, and stronger operational support.
...
In SKU B, audit export, acceptance evidence, backup/restore checks, and operational records can be included. The exact outputs and recovery process are confirmed according to the customer environment.

### Q8. Can PrivaPlane be delivered locally and run on Mac or Linux servers?

The current delivery design focuses on local delivery into a customer-controlled environment. Candidate runtime environments include a MacBook or Mac Studio running a 7B-14B class local model, or a customer-provided Linux server with GPU.
...
This does not mean every Mac or every 7B-14B model will provide the same experience. The intended commitment is that these local delivery options can be reviewed and confirmed against the actual customer environment.
